...
Global Instance equiv_trace {X} : Equiv (Trace X) := ITree.Equivalence.eutt IO (string + X).

Global Instance functor_trace : Functor Trace := fun {A B} (f:A -> B) => (@ITree.map IO) _ _ (fmap f).
(* Global Instance monad_trace' : (@Monad.Monad Trace) := *)
(* { ret X x := Ret (inr x) *)
(* ; bind := fun {A B} (X : Trace A) (f : A -> Trace B) => ITree.Core.bind X *)
(*                                                                     (fun m => match m with *)
(*                                                                            | inl s => Ret (inl s) *)
(*                                                                            | inr x => f x *)
(*                                                                            end) *)
